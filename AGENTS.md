...
- The address is the Nexus Portal business profile's, read live (the copy saved at connect is the fallback). When it can't be read, the page shows a website field holding the address that failed, to correct and try again. The typed address is used for that suggestion only and never saved.
- The server fetches that address, so `websiteAddress` allows only http(s) domain names on the normal port: no IP literals, ports, logins, localhost or internal names. Redirects are followed by hand and each hop is checked the same way (`nextHop`). A public name that resolves to a private address is the accepted limit.
